...
export class MangaKatanaScraper extends BaseScraper {
  ...
  getName(): string { return "MangaKatana"; }
  getBaseUrl(): string { return this.BASE_URL; }
  canHandle(url: string): boolean { return url.includes("mangakatana.com"); }
  getType(): SourceType { return "aggregator"; }
  async search(query: string): Promise<SearchResult[]> {
    ...
    $("#book_list .item").each((_, element) => {
      ...
      if (chapterMatch) latestChapter = parseFloat(chapterMatch[1]);
      const lastUpdated = $item.find(".date").first().text().trim();
      results.push({ id, title, url, coverImage: coverImage || undefined, latestChapter, lastUpdated });
    });
  ...
  async extractMangaInfo(url: string): Promise<{ title: string; id: string }> {
    ...
    const urlMatch = url.match(/\/manga\/([^/]+)\.(\d+)/);
    const id = urlMatch ? `${urlMatch[1]}.${urlMatch[2]}` : Date.now().toString();
    return { title, id };
  }
  async getChapterList(mangaUrl: string): Promise<ScrapedChapter[]> {
    ...
    $(".chapters table tbody tr").each((_, element) => {
      ...
      chapters.push({ id: `${chapterNumber}`, number: chapterNumber, title: chapterText, url: fullUrl, lastUpdated: dateText || undefined });
    });
  ...
  async getChapterPages(chapterUrl: string): Promise<string[]> {
    ...
    const $ = cheerio.load(html);
    const pages: string[] = [];
    // MangaKatana carga las imágenes en #imgs o mediante un script con array de URLs
    const scriptContent = $("script").toArray()
      .map(el => $(el).html() || "")
      .find(s => s.includes("var ytaw=") || s.includes("data_images"));
    if (scriptContent) {
      const match = scriptContent.match(/var ytaw=\[([^\]]+)\]/);
      if (match) {
        const urls = match[1].split(",").map(u => u.replace(/['"]/g, "").trim()).filter(Boolean);
        return urls;
      }
    }
